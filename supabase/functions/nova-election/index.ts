...
const defaultPartyData: Record<string, {
  abbreviations: string[],
  aliases: string[],
  colors: { primary: string, secondary: string, light: string, dark: string }
}> = {
  'DEM': {
...
function transformElectionData(rawData: any[]): any {
  ...
  const races = Array.from(racesMap.entries()).map(([raceKey, raceRows]) => {
    ...
    const title = formatRaceTitle(
      firstRow.election_name,
      firstRow.race_type,
      firstRow.state_code,
      firstRow.fips_code,
      firstRow.division_type,
      firstRow.year
    );
  ...
  races.sort((a, b) => {
    ...
    const typeOrder: Record<string, number> = { 'PRESIDENTIAL': 0, 'SENATE': 1, 'HOUSE': 2, 'GOVERNOR': 3 };
  ...
  rawData.forEach(row => {
    ...
    if (!uniqueCandidatesMap.has(candidateId)) {
      uniqueCandidatesMap.set(candidateId, {
        ...
        birthPlace: row.bio_short, // Using bio_short for birthPlace
  ...
  rawData.forEach(row => {
    ...
    if (!partiesMap.has(partyCode)) {
      ...
      if (Array.isArray(row.party_abbreviations) && row.party_abbreviations.length > 0) {
        ...
        abbreviations = [row.party_short_name, ...abbreviations];
      }
      ...
      partiesMap.set(partyCode, {
        ...
        logo: undefined, // Not available in current data
