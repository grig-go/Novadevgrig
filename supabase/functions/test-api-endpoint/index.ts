  ...
  switch (source.type) {
    ...
    case 'file':
      ...
      if (fileConfig?.url) {
        ...
        } else if (fileConfig.format === 'csv' || fileConfig.format === 'tsv') {
          const delimiter = fileConfig.format === 'tsv' ? '\t' : ',';
          return parseCSV(fileContent, {
            delimiter,
            hasHeaders: fileConfig.hasHeaders ?? true,
            customHeaders: fileConfig.customHeaders,
            headerRowNumber: fileConfig.headerRowNumber || 1  // NEW
          });
        }
        return fileContent;
      }
...
function parseCSV(content: string, options: any = {}) {
  const { delimiter = ',', hasHeaders = true, customHeaders, headerRowNumber = 1 } = options;
  ...
  // Calculate skip rows (headerRowNumber is 1-based)
  const skipRows = Math.max(0, headerRowNumber - 1);
  ...
  if (hasHeaders && rows.length > skipRows) {
    ...
    // No headers mode - skip specified rows and use remaining as data
  ...
  console.log('CSV Parser - Skip rows:', skipRows);
  console.log('CSV Parser - Header row index:', skipRows);
  console.log('CSV Parser - Headers:', headers);
  console.log('CSV Parser - Total data rows:', dataRows.length);
