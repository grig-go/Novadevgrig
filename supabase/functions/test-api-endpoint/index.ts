...
serve(async (req) => {
  ...
  try {
    ...
    // Return test results
    // Only parse JSON responses, leave XML/RSS/CSV as strings
    let responseData;
    if (formattedResponse.body) {
      if (formattedResponse.contentType === 'application/json') {
        responseData = JSON.parse(formattedResponse.body);
      } else {
        // For XML, RSS, CSV, return the string as-is
        responseData = formattedResponse.body;
      }
    } else {
...
`;
  ...
  // Extract items from data sources
  let items: any[] = [];

  // If data is an object with data source IDs as keys, extract items from each source
  if (typeof data === 'object' && !Array.isArray(data)) {
    for (const sourceId in data) {
      const sourceData = data[sourceId];
      // Check common paths for items (articles, items, data, results, etc.)
      if (sourceData.articles && Array.isArray(sourceData.articles)) {
        items = items.concat(sourceData.articles);
      } else if (sourceData.items && Array.isArray(sourceData.items)) {
        items = items.concat(sourceData.items);
      } else if (Array.isArray(sourceData)) {
        items = items.concat(sourceData);
      }
    }
  ...
  if (items.length > 0) {
    for (const item of items) {
      ...
      if (item.link || item.url) {
        rss += `      <link>${escapeXml(item.link || item.url)}</link>\n`;
      }
      if (item.pubDate || item.publishedAt) {
        rss += `      <pubDate>${new Date(item.pubDate || item.publishedAt).toUTCString()}</pubDate>\n`;
      }
      rss += '    </item>\n';
    }
  }
