...
export async function runTask(params: TaskParams) {
  try {
    ...
    const extractTrendValue = (section: string, name: string): number => {
    ...
    const extractSignedValue = (section: string, name: string): number => {
      const regex = new RegExp(`<trend\\s+name="${name}"\\s+value="([^"]+)"`, 'i');
      const match = section.match(regex);
    ...
    while ((partyMatch = partyRegex.exec(bopData)) !== null) {
      ...
      let { data: existingParty } = await supabase
        .from('bop_party_results')
        .select('id')
        .eq('election_result_id', electionResultId)
        .eq('party_name', partyName)
        .maybeSingle();
      
      let partyResultId: number;
      ...
      if (netChangeMatch) {
        ...
        // Upsert net change
        let { data: existingNetChange } = await supabase
          .from('bop_net_changes')
          .select('id')
    ...
    if (insufficientVoteMatch) {
      ...
      let { data: existingIV } = await supabase
        .from('bop_insufficient_vote_details')
        .select('id')
        .eq('election_result_id', electionResultId)
        .maybeSingle();
