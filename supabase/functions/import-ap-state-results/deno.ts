...
export async function runTask(params: TaskParams) {
  try {
    const { 
      electionDate, 
      officeID = 'P',
    ...
    if (!electionDate) throw new Error('electionDate is required');
    ...
    const env = config({ path: ".env" });   //.env based on where you run the deno command

    const supabaseUrl = env.VITE_SUPABASE_URL;
    const supabaseKey = env.VITE_SUPABASE_ANON_KEY;
    const apApiKey = env.AP_API_KEY;
    ...
    const { data: apHistory, error: apError } = await supabase
      .from('e_ap_call_history')
      .select('id, nextrequest')
      .eq('officeid', officeID)
      .eq('resultstype', resultsType)
      .eq('level', 'state')
      .eq('electiondate', electionDate)
      .limit(1)
      .maybeSingle()

    if (apError) {
      return new Response(JSON.stringify({ error: apError.message }), { status: 500 })
    }
    ...
    if (apData.nextrequest) {
      ...
      const { data: apInsertHistory, error: apError } = await supabase
        .from('e_ap_call_history')
        .upsert([aprow], { onConflict: ['id'] })
        .select() // optional — returns inserted row(s)

      if (apError) {
        console.error('Error inserting call history:', apError)
        return new Response(JSON.stringify({ apError }), { status: 500 })
      }
    }
    ...
    const { data: existParties } = await supabase
    .from('e_parties')
    .select('abbreviation, color_hex, name')
    .in('abbreviation', ['GOP', 'Dem', 'Grn', 'Lib', 'Ind'])
    ...
    for (let i = 0; i < existParties.length; i++) {
      ...
      if (!existingParties[abbreviation]) {
        existingParties[abbreviation] = { name, color_hex };
      }
    } 

    const partyData = [
      { code: 'GOP', name: existingParties['GOP']?.name ? existingParties['GOP'].name : 'Republican Party', color: existingParties['GOP']?.color_hex ? existingParties['GOP'].color_hex : '#FF0000' },
      { code: 'Dem', name: existingParties['Dem']?.name ? existingParties['Dem'].name : 'Democratic Party', color: existingParties['Dem']?.color_hex ? existingParties['Dem'].color_hex : '#0000FF' },
      { code: 'Grn', name: existingParties['Grn']?.name ? existingParties['Grn'].name : 'Green Party', color: existingParties['Grn']?.color_hex ? existingParties['Grn'].color_hex : '#2fa82f' },
      { code: 'Lib', name: existingParties['Lib']?.name ? existingParties['Lib'].name : 'Libertarian Party', color: existingParties['Lib']?.color_hex ? existingParties['Lib'].color_hex : '#efcb0a' },
      { code: 'Ind', name: existingParties['Ind']?.name ? existingParties['Ind'].name : 'Independent', color: existingParties['Ind']?.color_hex ? existingParties['Ind'].color_hex : '#808080' },
    ].map(p => ({
    ...
    await supabase
      .from('e_parties')
      .upsert(partyData, { onConflict: 'party_id' })

    // Get party mapping (abbreviation -> UUID)
    const { data: parties } = await supabase
      .from('e_parties')
      .select('id, abbreviation')
      .in('party_id', partyData.map(p => p.party_id))

    const partyMap = Object.fromEntries(
      parties?.map(p => [p.abbreviation, p.id]) || []
    )
    ...
    for (const race of apData.races || []) {
      ...
      if (stateCode !== 'US') {
        ...
        statesData.push({
          ...
          metadata: { ap_state_id: race.stateID }
        })
      }
      else {
        statesData.push({
    ...
    const uniqueStates = statesData.filter((v,i,a) => 
      a.findIndex(e => e.division_id === v.division_id) === i
    )
    ...
    if (uniqueStates.length > 0) {
      ...
      const { error: statesError } = await supabase
        .from('e_geographic_divisions')
        .upsert(uniqueStates, { onConflict: 'division_id' })
    ...
    if (uniqueStates.length > 0) {
      const { data: divisionRecords, error: divisionError } = await supabase
        .from('e_geographic_divisions')
        .select('id, division_id')
        .in('division_id', uniqueStates.map(s => s.division_id))
      ...
      divisionIdMap = Object.fromEntries(
        divisionRecords?.map(d => [d.division_id, d.id]) || []
      )
    ...
    if (candidatesArray.length > 0) {
      ...
      const { error: candidatesError } = await supabase
        .from('e_candidates')
        .upsert(candidatesArray, { onConflict: 'candidate_id' })
    ...
    for (let i = 0; i < candidateIds.length; i += BATCH_SIZE) {
      ...
      const { data, candError } = await supabase
        .from('e_candidates')
    ...
    const candidateIdMap = Object.fromEntries(
      candidateRecords?.map(c => [c.candidate_id, c.id]) || []
    )
    ...
    for (const race of apData.races || []) {
      ...
      racesData.push({
        race_id: raceId,
        election_id: election.id, // UUID from election record
        division_id: divisionUUID, // UUID from divisions (or null for national)
        name: race.seatName 
          ? `${race.officeName} - ${stateName} ${race.seatName}` 
          : `${race.officeName} - ${stateName}`,
        type: raceType,
    ...
    const { data: raceRecords } = await supabase
      .from('e_races')
      .select('id, race_id')
      .in('race_id', racesData.map(r => r.race_id))

    const raceIdMap = Object.fromEntries(
      raceRecords?.map(r => [r.race_id, r.id]) || []
    )
      ...
      for (const candidate of reportingUnit.candidates || []) {
        ...
        if (candidateUUID) {
          raceCandidatesData.push({
            race_id: raceUUID, // UUID
            candidate_id: candidateUUID, // UUID
    ...
    if (raceCandidatesData.length > 0) {
      ...
      const { error: rcError } = await supabase
        ...
        .upsert(raceCandidatesData, { onConflict: 'race_id,candidate_id' })
    ...
    for (const race of apData.races || []) {
      ...
      raceResultsData.push({
        race_id: raceUUID, // UUID
        division_id: divisionUUID, // UUID or null
        ...
        called_by_source: 'ap',
        winner_candidate_id: winnerCandidateUUID, // UUID
    ...
    if (raceResultsData.length > 0) {
      ...
      const { data: raceResultRecords, error: rrError } = await supabase
        .from('e_race_results')
        .upsert(raceResultsData, { 
      ...
      for (const race of apData.races || []) {
        ...
        for (const candidate of reportingUnit.candidates || []) {
          ...
          candidateResultsData.push({
            race_result_id: raceResultUUID, // UUID
            candidate_id: candidateUUID, // UUID
      ...
      if (candidateResultsData.length > 0) {
        ...
        for (let i = 0; i < candidateResultsData.length; i += batchSize) {
          ...
          const { error: crError } = await supabase
            ...
            .upsert(batch, { onConflict: 'race_result_id,candidate_id' })
    ...
    const { data: ds, error: dsError } = await supabase
      .from('data_sources')
      .select('id')
      .eq('name', 'AP Election API')
      .limit(1)

    if (dsError || !ds || ds.length === 0) {
      return new Response(JSON.stringify({ error: 'Failed to find data_source_id', details: dsError }), {
        status: 400,
      })
    }
    ...
    const record = {
      ...
      priority: 1,
      active: true,
      config: {},       // only used on insert
      field_mapping: { level: 'state', duration: `${duration}s` },// only used on insert
