...
export async function runTask(params: TaskParams) {
  ...
  try {
    const { electionDate, officeID, resultsType, raceType, raceName } = params
    ...
    const { data: apHistory, error: apError } = await supabaseClient
    ...
    if (apData.nextrequest) {
      ...
      const { data: apInsertHistory, error: apError } = await supabaseClient
        .from('e_ap_call_history')
    ...
    let election = await supabaseClient
      .from('e_elections')
      .select('id')
    ...
    if (!election.data) {
      ...
      const { data: newElection, error: electionError } = await supabaseClient
        .from('e_elections')
        .insert({
    ...
    const { data: existParties } = await supabaseClient
    .from('e_parties')
    ...
    const partyData = {
      GOP: { code: 'GOP', name: existingParties['GOP']?.name ? existingParties['GOP'].name : 'Republican Party', color: existingParties['GOP']?.color_hex ? existingParties['GOP'].color_hex : '#FF0000' },
      Dem: { code: 'Dem', name: existingParties['Dem']?.name ? existingParties['Dem'].name : 'Democratic Party', color: existingParties['Dem']?.color_hex ? existingParties['Dem'].color_hex : '#0000FF' },
      Grn: { code: 'Grn', name: existingParties['Grn']?.name ? existingParties['Grn'].name : 'Green Party', color: existingParties['Grn']?.color_hex ? existingParties['Grn'].color_hex : '#2fa82f' },
      Lib: { code: 'Lib', name: existingParties['Lib']?.name ? existingParties['Lib'].name : 'Libertarian Party', color: existingParties['Lib']?.color_hex ? existingParties['Lib'].color_hex : '#efcb0a' },
      Ind: { code: 'Ind', name: existingParties['Ind']?.name ? existingParties['Ind'].name : 'Independent', color: existingParties['Ind']?.color_hex ? existingParties['Ind'].color_hex : '#808080' },
    };
    ...
    async function batchUpsert(table: string, data: any[], conflictKey: string) {
    ...
    const { data: parties } = await supabaseClient
      .from('e_parties')
      .select('id, party_id')
      .in('party_id', partiesToUpsert.map(p => p.party_id))
    
    const partyUuidMap = new Map<string, string>()
    ...
    // Initialize maps
    const divisionUuidMap = new Map<string, string>()

    // Arrays for batch processing
    const divisionsToUpsert: any[] = []
    const racesToUpsert: any[] = []
    const raceResultsToUpsert: any[] = []
    const candidateResultsToUpsert: any[] = []
    const raceCandidatesToUpsert: any[] = []
    
    const raceSet = new Set<string>()
    const raceResultKeys = new Set<string>()
    const raceCandidateKeys = new Set<string>()
    let districtCount = 0
    ...
    if (divisionsToUpsert.length > 0) {
      ...
      for (let i = 0; i < divisionIds.length; i += BATCH_SIZE_SELECT) {
        ...
        const { data: newDivs, error: newDivsError } = await supabaseClient
    ...
    const raceUuidMap = new Map<string, string>()
    ...
    const uuidToRaceId = new Map<string, string>()
    for (const [raceId, uuid] of raceUuidMap.entries()) {
      uuidToRaceId.set(uuid, raceId)
    }

    const uuidToDivisionId = new Map<string, string>()
    for (const [divId, uuid] of divisionUuidMap.entries()) {
      uuidToDivisionId.set(uuid, divId)
    }

    const raceUuidList = [...raceUuidMap.values()]
    const rrUuidMap = new Map<string, string>()
    
    for (let i = 0; i < raceUuidList.length; i += BATCH_SIZE_SELECT) {
      const batch = raceUuidList.slice(i, i + BATCH_SIZE_SELECT)

      const { data: rrData, error: rrError } = await supabaseClient
        .from('e_race_results')
        .select('id, race_id, division_id')
        .in('race_id', batch)
        .eq('reporting_level', 'district')
    ...
    const candidateResultsMap = new Map<string, any>()
    ...
    const { data: ds, error: dsError } = await supabaseClient
      .from('data_sources')
      .select('id')
    ...
    const record = {
      ...
      field_mapping: { level: 'county', duration: `${duration}s` },// only used on insert
    ...
    const { data: dsData, error } = await supabaseClient
