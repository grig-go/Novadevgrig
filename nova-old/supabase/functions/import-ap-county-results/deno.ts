...
export async function runTask(params: TaskParams) {
  ...
  try {
    ...
    const { data: existingCandidates } = await supabaseClient
      .from('e_candidates')
      ...
      for (const unit of countyUnits) {
        ...
        if (!raceSet.has(raceIdStr)) {
          racesToUpsert.push({
            ...
            division_id: divisionIdStr,  // String for now
        ...
        if (!raceResultKeys.has(raceResultKey)) {
          raceResultsToUpsert.push({
            race_id: raceIdStr,  // String for now
            division_id: divisionIdStr,  // String for now
    ...
    const { data: verifyRR, error: verifyError } = await supabaseClient
      .from('e_race_results')
      .select('id, race_id')
      .limit(3)
