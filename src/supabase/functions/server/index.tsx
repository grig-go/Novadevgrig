...
app.get("/make-server-cbef71cf/health", (c) => {
  return c.json({ status: "ok", build: BUILD_ID });
});
...
app.post("/make-server-cbef71cf/test-provider", async (c) => {
  try {
    ...
    let testResult: any = { success: false, message: "", details: null };
            ...
            if (!response.ok) {
              ...
              testResult = {
                ...
                details: {
                  ...
                  sampleLeagues: data.data?.slice(0, 3).map((l: any) => l.name),
                },
              };
            }
          } catch (error) {
 ...
 */
function jsonErr(c: any, status: number, code: string, detail?: unknown) {
  console.error(`[${code}]`, detail ?? '');
  return c.json({ ok: false, error: code, detail: String(detail ?? '') }, status);
}
...
app.get("/make-server-cbef71cf/feeds", async (c) => {
  try {
    ...
    return c.json({ ok: true, feeds: feeds || [] });
  } catch (error) {
    console.error("Error fetching feeds:", error);
    return c.json({ error: "Failed to fetch feeds", details: String(error) }, 500);
  }
});
...
app.put("/make-server-cbef71cf/feeds/:id", async (c) => {
  try {
    ...
    const updatedFeed = { ...existingFeed, ...body, updatedAt: new Date().toISOString() };
    await kv.set(`feed:${id}`, updatedFeed);
    
    return c.json({ ok: true, success: true, feed: updatedFeed });
  } catch (error) {
...
app.get("/make-server-cbef71cf/data-providers", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: "Failed to fetch data providers", details: error.message }, 500);
    }
    ...
    return c.json({ ok: true, providers, stats });
  } catch (error) {
    console.error("Error fetching data providers:", error);
    return c.json({ error: "Failed to fetch data providers", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/finance-providers/reveal", async (c) => {
  try {
    ...
    const { data: financeProviders, error } = await supabase
      .from("data_providers")
      .select("*")
      .eq("category", "finance")
      .order("name", { ascending: true });
    ...
    if (error) {
      ...
      return c.json({ error: "Failed to fetch finance providers", details: error.message }, 500);
    }
    ...
    const providers = (financeProviders || []).map(dp => ({
      ...
      category: dp.category,
      description: dp.description,
      apiKey: dp.api_key || null, // Unmasked
      apiSecret: dp.api_secret || null, // Unmasked
      apiKeyConfigured: !!dp.api_key,
      apiSecretConfigured: !!dp.api_secret,
      status: dp.is_active ? "active" : "inactive",
      isActive: dp.is_active,
    ...
    return c.json({ error: "Failed to reveal credentials", details: String(error) }, 500);
  }
});
...
app.put("/make-server-cbef71cf/data-providers/:id", async (c) => {
  try {
    ...
    const category = body.category || (
      id.startsWith("weather_provider:") ? "weather" :
      id.startsWith("sports_provider:") ? "sports" :
      id.startsWith("news_provider:") ? "news" :
      id.startsWith("finance_provider:") ? "finance" :
      "ai"
    );
    ...
    if (category === "ai") {
      ...
      const prefix = 
        category === "weather" ? "weather_provider:" :
        category === "sports" ? "sports_provider:" :
        category === "finance" ? "finance_provider:" :
        "news_provider:";
      ...
      const providerType = body.type || (
        key.includes('weatherapi') ? 'weatherapi' :
        key.includes('sportsradar') ? 'sportsradar' :
        key.includes('sportmonks') ? 'sportmonks' :
        key.includes('newsapi') ? 'newsapi' :
        key.includes('newsdata') ? 'newsdata' :
        'unknown'
      );
      ...
      if (category === "weather") {
        ...
        updatedProvider.baseUrl = baseProvider.baseUrl || (
          providerType === 'sportsradar' ? 'https://api.sportradar.com' : 'https://api.sportmonks.com/v3'
        );
        ...
        updatedProvider.baseUrl = baseProvider.baseUrl || (
          providerType === 'alpaca' ? ALPACA_TRADING_BASE : 'https://api.coingecko.com/api/v3'
        );
...
async function initializeDefaultFinanceProviders() {
  try {
    ...
    const { data: existingAlpaca } = await supabase
      .from("data_providers")
      .select("id")
      .eq("id", "finance_provider:alpaca")
      .single();
    ...
    if (!existingAlpaca) {
      ...
      await supabase.from("data_providers").insert({
        ...
        api_version: "v2",
        config: JSON.stringify({ dataTypes: ["stocks", "etfs"] }),
      });
    ...
    const { data: existingCoinGecko } = await supabase
      .from("data_providers")
      .select("id")
      .eq("id", "finance_provider:coingecko")
      .single();
    ...
    if (!existingCoinGecko) {
      ...
      await supabase.from("data_providers").insert({
        ...
        api_version: "v3",
        config: JSON.stringify({ dataTypes: ["crypto"] }),
      });
...
app.post("/make-server-cbef71cf/ai-providers/initialize", async (c) => {
  try {
    ...
    console.error("Error initializing AI providers:", error);
    return c.json({ error: "Failed to initialize AI providers", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/ai-providers/migrate-from-kv", async (c) => {
  try {
    ...
    const upserts = kvProviders.map((provider: any) => ({
      ...
      available_models: Array.isArray(provider.availableModels || provider.available_models) 
        ? (provider.availableModels || provider.available_models) 
        : [],
      enabled: provider.enabled ?? true,
    ...
    return c.json({ 
      ...
      providers: upserts.map(p => ({ id: p.id, name: p.name }))
    });
  } catch (error) {
...
app.post("/make-server-cbef71cf/ai-providers/fetch-models", async (c) => {
  ...
  if (!providerName) {
    return c.json({ ok:false, error:"MISSING_PROVIDER_NAME", detail:"providerName is required" }, 400);
  }
  if (!apiKey && providerName !== "claude") {
    // Claude returns a static list below; allow missing key only there
    return c.json({ ok:false, error:"MISSING_API_KEY", detail:"apiKey is required" }, 400);
  }
  ...
  switch (providerName) {
    case "openai": {
      ...
      if (!r.ok) {
        ...
        return c.json({ ok:false, error:"OPENAI_BAD_STATUS", detail:`${r.status}: ${t}` }, 502);
      }
      ...
      const getModelInfo = (modelId: string) => {
        if (modelId.includes("gpt-4o")) {
          return { name: "GPT-4o", description: "Multimodal flagship model" };
        } else if (modelId.includes("gpt-4-turbo")) {
          return { name: "GPT-4 Turbo", description: "Fast and powerful" };
        } else if (modelId.includes("gpt-4")) {
          return { name: "GPT-4", description: "Advanced reasoning" };
        } else if (modelId.includes("gpt-3.5-turbo")) {
          return { name: "GPT-3.5 Turbo", description: "Fast and efficient" };
    ...
    case "gemini": {
      ...
      if (!r.ok) {
        const t = await r.text();
        return c.json({ ok:false, error:"GEMINI_BAD_STATUS", detail:`${r.status}: ${t}` }, 502);
      }
      const data = await r.json();
      ...
      const uniqueById = allModels.filter((m: any, index: number, self: any[]) => 
        self.findIndex((t: any) => t.id === m.id) === index
      );
      
      // For models with same display name, prefer experimental/latest versions
      const uniqueByName = uniqueById.filter((m: any, index: number, self: any[]) => {
        const sameNameIndex = self.findIndex((t: any) => t.name === m.name);
      ...
      models = uniqueByName.map(({ rawName, version, ...model }: any) => model);
      break;
    }
    ...
    case "mistral": {
      ...
      if (!r.ok) {
        const t = await r.text();
        return c.json({ ok:false, error:"MISTRAL_BAD_STATUS", detail:`${r.status}: ${t}` }, 502);
      }
      const data = await r.json();
      
      // Helper function to get friendly descriptions
      const getMistralInfo = (modelId: string) => {
        if (modelId.includes("mistral-large")) {
          return { description: "Most capable model for complex tasks" };
        } else if (modelId.includes("mistral-medium")) {
          return { description: "Balanced performance and cost" };
        } else if (modelId.includes("mistral-small")) {
          return { description: "Fast and cost-effective" };
        } else if (modelId.includes("mistral-tiny")) {
          return { description: "Fastest model for simple tasks" };
        }
        return { description: "" };
      };
    ...
    case "claude": {
      ...
      models = [
        { id:"claude-3-7-sonnet-20250219", name:"Claude 3.7 Sonnet", description:"Latest model with extended thinking and improved performance", type:"text" },
        { id:"claude-3-5-sonnet-20241022", name:"Claude 3.5 Sonnet (New)", description:"Updated version with better coding and analysis", type:"text" },
        { id:"claude-3-5-sonnet-20240620", name:"Claude 3.5 Sonnet", description:"Balanced performance and speed", type:"text" },
        { id:"claude-3-opus-20240229",     name:"Claude 3 Opus",     description:"Most capable model for complex tasks", type:"text" },
        { id:"claude-3-sonnet-20240229",   name:"Claude 3 Sonnet",   description:"Balanced intelligence and speed", type:"text" },
        { id:"claude-3-haiku-20240307",    name:"Claude 3 Haiku",    description:"Fastest model for simple tasks", type:"text" },
      ];
      break;
    }
...
app.get("/make-server-cbef71cf/weather-locations", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: "Failed to fetch weather locations", details: error.message }, 500);
    }
    
    return c.json({ locations: locations || [] });
  } catch (error) {
    console.error("Error fetching weather locations:", error);
    return c.json({ error: "Failed to fetch weather locations", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/weather-locations", async (c) => {
  try {
    ...
    return c.json({ ok: true, success: true, location: data });
  } catch (error) {
...
app.delete("/make-server-cbef71cf/weather-locations/:id", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to delete weather location", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/weather-providers", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: "Failed to fetch weather providers", details: String(error) }, 500);
    }
...
app.get("/make-server-cbef71cf/weather-locations/search", async (c) => {
  try {
    ...
    if (providerError) {
      ...
      return c.json({ error: "Failed to fetch weather provider configuration" }, 500);
    }
    
    const activeProvider = providers?.[0];
    if (!activeProvider) {
      return c.json({ error: "No active weather provider configured" }, 400);
    }
    ...
    if (!apiKey) {
      return c.json({ error: "Weather provider API key not configured" }, 400);
    }
    ...
    if (!response.ok) {
      ...
      return c.json({ error: "Failed to search locations", details: errorText }, response.status);
    }
    ...
    return c.json({ error: "Failed to search locations", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/weather-ai-insights", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to fetch AI insights", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/weather/cached/:locationId", async (c) => {
  try {
    ...
    const [currentResult, airQualityResult, hourlyResult, dailyResult, alertsResult] = await Promise.all([
      ...
      supabase.from("weather_hourly_forecast").select("*").eq("location_id", locationId).order("forecast_time", { ascending: true }),
      supabase.from("weather_daily_forecast").select("*").eq("location_id", locationId).order("forecast_date", { ascending: true }),
      supabase.from("weather_alerts").select("*").eq("location_id", locationId).order("start_time", { ascending: false }),
    ]);
    ...
    const response = {
      ...
      location: {
        ...
        custom_name: location.custom_name, // Show if user has customized
      },
      current: {
        asOf: current.as_of,
        summary: current.summary,
        icon: current.icon,
        temperature: { value: current.temperature_value, unit: current.temperature_unit || "°C" },
        feelsLike: { value: current.feels_like_value, unit: current.feels_like_unit || "°C" },
        dewPoint: { value: current.dew_point_value, unit: current.dew_point_unit || "°C" },
        humidity: current.humidity,
        pressure: { value: current.pressure_value, unit: current.pressure_unit || "mb" },
        wind: {
          speed: { value: current.wind_speed_value, unit: current.wind_speed_unit || "kph" },
          gust: current.wind_gust_value ? { value: current.wind_gust_value, unit: current.wind_gust_unit || "kph" } : null,
          direction_deg: current.wind_direction_deg,
          direction_cardinal: current.wind_direction_cardinal,
        },
        visibility: { value: current.visibility_value, unit: current.visibility_unit || "km" },
      ...
      hourly: {
        items: hourlyResult.data?.map((h: any) => ({
          time: h.forecast_time,
          summary: h.summary,
          icon: h.icon,
          temperature: { value: h.temperature_value, unit: h.temperature_unit || "°C" },
          feelsLike: { value: h.feels_like_value, unit: h.feels_like_unit || "°C" },
          dewPoint: { value: h.dew_point_value, unit: h.dew_point_unit || "°C" },
          humidity: h.humidity,
          wind: {
            speed: { value: h.wind_speed_value, unit: h.wind_speed_unit || "kph" },
            direction_deg: h.wind_direction_deg,
          },
          precipProbability: h.precip_probability,
          precipAmount: { value: h.precip_intensity_value, unit: h.precip_intensity_unit || "mm" },
          cloudCover: h.cloud_cover,
          uvIndex: h.uv_index,
        })) || [],
      },
      daily: {
        items: dailyResult.data?.map((d: any) => ({
          date: d.forecast_date,
          summary: d.summary,
          icon: d.icon,
          tempMax: { value: d.temp_max_value, unit: d.temp_max_unit || "°C" },
          tempMin: { value: d.temp_min_value, unit: d.temp_min_unit || "°C" },
          precipProbability: d.precip_probability,
          precipAmount: { value: d.precip_accumulation_value, unit: d.precip_accumulation_unit || "mm" },
...
function convertToISOTimestamp(weatherApiTimestamp: string): string {
...
app.get("/make-server-cbef71cf/weather-data", async (c) => {
  try {
    ...
    if (locationsError) {
      ...
      return c.json({ error: "Failed to fetch weather locations", details: locationsError.message }, 500);
    }
    
    console.log(`Found ${locations?.length || 0} active locations from database`);
    
    if (!locations || locations.length === 0) {
      return c.json({ error: "No active weather locations configured. Please add locations first." }, 400);
    }
    ...
    const providerConfig = typeof activeProvider.config === 'string' 
    ...
    const weatherDataPromises = validLocations.map(async (location) => {
      try {
        ...
        await supabase
          ...
          .upsert(currentWeather, { onConflict: "lat,lon" });
        ...
        const weatherData = {
          ...
          location: {
            id: `${lat},${lon}`,
            name: location.name,  // Don't apply custom_name here, let frontend handle it
            admin1: location.admin1 || currentData.location.region,
            country: location.country || currentData.location.country,
            lat,
            lon,
          ...
          current: {
            ...
            summary: currentData.current.condition.text,
            icon: currentData.current.condition.icon,
            temperature: { value: currentData.current.temp_c, unit: "°C" },
            feelsLike: { value: currentData.current.feelslike_c, unit: "°C" },
            dewPoint: { value: currentData.current.dewpoint_c || 0, unit: "°C" },
            humidity: currentData.current.humidity,
            pressure: { value: currentData.current.pressure_mb, unit: "mb" },
            wind: {
              speed: { value: currentData.current.wind_kph, unit: "km/h" },
              gust: { value: currentData.current.gust_kph, unit: "km/h" },
              direction_deg: toInt(currentData.current.wind_degree) || 0,
              direction_cardinal: currentData.current.wind_dir,
            },
            visibility: { value: currentData.current.vis_km, unit: "km" },
            cloudCover: currentData.current.cloud,
            uvIndex: currentData.current.uv,
            precipLastHr: { value: currentData.current.precip_mm, unit: "mm" },
            precipType: "rain",
            snowDepth: { value: 0, unit: "cm" },
            sun: {
          ...
          hourly: {
            stepHours: 1,
            items: forecastData.forecast.forecastday.flatMap((day: any) =>
              day.hour.slice(0, 24).map((hour: any) => ({
                time: convertToISOTimestamp(hour.time),
                summary: hour.condition.text,
                icon: hour.condition.icon,
                temperature: { value: hour.temp_c, unit: "°C" },
                feelsLike: { value: hour.feelslike_c, unit: "°C" },
                dewPoint: { value: hour.dewpoint_c || 0, unit: "°C" },
                humidity: hour.humidity,
                wind: {
                  speed: { value: hour.wind_kph, unit: "km/h" },
                  gust: { value: hour.gust_kph, unit: "km/h" },
                  direction_deg: toInt(hour.wind_degree) || 0,
                  direction_cardinal: hour.wind_dir,
                },
                pressure: { value: hour.pressure_mb, unit: "mb" },
                cloudCover: hour.cloud,
                precipProbability: hour.chance_of_rain / 100,
                precipIntensity: { value: hour.precip_mm, unit: "mm" },
                uvIndex: hour.uv,
              }))
            ).slice(0, 48),
          },
          daily: {
            items: forecastData.forecast.forecastday.map((day: any) => ({
              date: day.date,
              summary: day.day.condition.text,
              icon: day.day.condition.icon,
              tempMax: { value: day.day.maxtemp_c, unit: "°C" },
              tempMin: { value: day.day.mintemp_c, unit: "°C" },
              ...
              precipType: "rain",
              precipAccumulation: { value: day.day.totalprecip_mm, unit: "mm" },
              snowAccumulation: { value: day.day.totalsnow_cm || 0, unit: "cm" },
              wind: {
                speedAvg: { value: day.day.maxwind_kph, unit: "km/h" },
                gustMax: { value: day.day.maxwind_kph, unit: "km/h" },
                direction_deg: 0,
              },
            })),
          },
          alerts: forecastData.alerts?.alert?.map((alert: any) => ({
    ...
    return c.json({ error: "Failed to fetch weather data", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/news-providers", async (c) => {
  try {
    ...
    return c.json({ ok: true, providers: maskedProviders });
  } catch (error) {
...
app.delete("/make-server-cbef71cf/news-providers/:id", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to delete news provider", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/news-articles", async (c) => {
  try {
    ...
    if (!providersWithCredentials || providersWithCredentials.length === 0) {
      return c.json({ articles: [], message: 'No news providers provided' });
    }

    console.log(`[NEWS] Fetching from ${providersWithCredentials.length} provider(s):`, providersWithCredentials.map((p: any) => p.type));
    ...
    try {
      ...
      const articlesToSave = limitedArticles.map((article: any) => ({
...
app.get("/make-server-cbef71cf/news-articles/stored", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to fetch stored articles', details: error.message }, 500);
    }
...
app.get("/make-server-cbef71cf/news-ai-insights", async (c) => {
  try {
    ...
    const insights = (results || []).map((r: any) => r.value);
    
    // Sort by created_at descending (newest first)
    const sortedInsights = insights.sort((a: any, b: any) => {
      return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
    });
    
    return c.json({ ok: true, insights: sortedInsights });
  } catch (error) {
...
app.get("/make-server-cbef71cf/sports/teams", async (c) => {
  return c.json({ ok: false, error: "SPORTS_FEATURE_NOT_CONFIGURED" }, 501);
});
...
app.get("/make-server-cbef71cf/sports/standings/:leagueId", async (c) => {
  try {
    ...
    if (!league.active_season_id) {
      ...
      return c.json({ error: "League has no active season configured" }, 400);
    }
    ...
    if (providerError || !providers || providers.length === 0) {
      ...
      return c.json({ error: "No active sports provider configured" }, 400);
    }
    ...
    if (!apiKey) {
      ...
      return c.json({ error: "Provider API key not configured" }, 400);
    }
    ...
    if (providerType === "sportmonks") {
      ...
      return c.json({ error: "Sportsradar standings not yet implemented" }, 501);
    } else {
...
app.get("/make-server-cbef71cf/sports/providers/active", async (c) => {
  try {
    ...
    if (listError) {
      ...
      return c.json({ error: "Failed to list sports providers" }, 500);
    }
    ...
    const { data: providerDetails, error: rpcError } = await supabase
      .rpc('get_provider_details', { p_id: providers[0].id });
    ...
    return c.json({ error: "Failed to fetch sports providers", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/sports/sportmonks/soccer/leagues", async (c) => {
  try {
    ...
    if (rpcError || !providerDetails || providerDetails.length === 0) {
      ...
      return c.json({ error: "Failed to fetch provider credentials" }, 500);
    }
    
    const provider = providerDetails[0];
    
    if (!provider.api_key) {
      return c.json({ error: "SportMonks API key not configured" }, 400);
    }
    ...
    if (!response.ok) {
      ...
      return c.json({ error: `SportMonks API error: ${response.status}` }, response.status);
    }
    ...
    return c.json({ error: "Failed to fetch SportMonks leagues", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/sports/sportsradar/soccer/competitions", async (c) => {
  try {
    ...
    if (listError || !providers || providers.length === 0) {
      ...
      return c.json({ error: "No active Sportsradar provider configured" }, 400);
    }
    ...
    if (!provider.api_key) {
      return c.json({ error: "Sportsradar API key not configured" }, 400);
    }
    ...
    if (!response.ok) {
      ...
      return c.json({ error: `Sportsradar API error: ${response.status}` }, response.status);
    }
    ...
    return c.json({ error: "Failed to fetch Sportsradar competitions", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/sports/add-league", async (c) => {
  try {
    ...
    if (!leagueId) {
      return c.json({ error: "Missing leagueId parameter" }, 400);
    }
    ...
    if (providerType === "sportmonks") {
      ...
      try {
        ...
        async function fetchLast5(teamId: number): Promise<string> {
          try {
            ...
            return fixtures.map((fixture: any) => {
              ...
              const teamScore = scores.find((s: any) => 
                s.participant_id === teamId && s.description === "CURRENT"
              );
              const opponentScore = scores.find((s: any) => 
                s.participant_id === opponentParticipant.id && s.description === "CURRENT"
              );
        ...
        teams = await Promise.all(rawTeams.map(async (t: any) => {
          // Find standing for this team
          const standing = standings.find((s: any) => s.participant_id === t.id);
      ...
      try {
        ...
        // Find current season
        let currentSeason = seasons.find((s: any) => s.current === true);
        ...
        teams = rawCompetitors.map((c: any) => ({
          id: c.id, // Keep "sr:team:123" format
          league_id: leagueId,
          name: c.name,
          short_name: c.short_name || c.abbreviation || null,
          abbreviation: c.abbreviation || null,
          logo_url: null, // Sportsradar doesn't provide logos in competitors endpoint
          venue: null,
          city: null,
    ...
    return c.json({ error: "Failed to add league teams", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/sports/remove-league", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to remove league teams", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/sports/add-team", async (c) => {
  try {
    ...
    return c.json({ success: true, team: data });
  } catch (error) {
    console.error("Error adding sports team:", error);
    return c.json({ error: "Failed to add sports team", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/sports-teams", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to fetch sports teams", details: String(error) }, 500);
  }
});
...
app.delete("/make-server-cbef71cf/sports-teams/:id", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to delete sports team", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/sports-data/games", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to fetch games", details: String(error) }, 500);
  }
});
...
app.get("/make-server-cbef71cf/sports-data/tournaments", async (c) => {
  try {
    ...
    const tournaments = leagues?.map(league => ({
      id: String(league.id), // Convert BIGINT to string
      name: league.name,
    ...
    return c.json({ error: "Failed to fetch tournaments", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/sports/save-leagues", async (c) => {
  try {
    ...
    if (!Array.isArray(leagues) || leagues.length === 0) {
      return c.json({ error: "Invalid request: leagues array required" }, 400);
    }
    ...
    const leagueRecords = leagues.map((league: any) => ({
      id: league.id, // SportMonks league ID (BIGINT)
      name: league.name,
      type: league.abbrev || 'league', // Use abbrev as type
      sport: league.sport || 'football',
      api_source: 'sportmonks',
      logo: null, // Can be enhanced later
      country_name: null, // Can be enhanced later
      season_id: league.seasonId || null, // BIGINT
      active_season_id: league.seasonId || null, // SportMonks uses same value
...
app.get("/make-server-cbef71cf/alpaca-stocks", async (c) => {
  try {
    ...
    return c.json({ error: "Failed to fetch Alpaca stocks", details: String(error) }, 500);
  }
});
...
app.post("/make-server-cbef71cf/alpaca-stocks/fetch", async (c) => {
  try {
    ...
    const { data: alpacaProvider } = await supabase
      .from("data_providers")
    ...
    if (!alpacaProvider || !alpacaProvider.is_active) {
      return c.json({ error: "Alpaca provider not configured or inactive" }, 400);
    }
    ...
    if (!alpacaKey || !alpacaSecret) {
      return c.json({ error: "Alpaca API credentials not set in database" }, 400);
    }
    ...
    if (!stocks || stocks.length === 0) {
      return c.json({ message: "No stocks configured" });
    }
    
    const symbols = stocks.map(s => s.symbol).join(",");
    
    const response = await fetch(
      `${ALPACA_DATA_BASE}/v2/stocks/snapshots?symbols=${symbols}`,
      {
        headers: {
    ...
    // Update database
    const updates = Object.entries(data).map(([symbol, snapshot]: [string, any]) => ({
      symbol,
    ...
    return c.json({ error: "Failed to fetch stock data", details: String(error) }, 500);
  }
});
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        response = await fetch(
          `${ALPACA_TRADING_BASE}/v2/assets?status=active&asset_class=us_equity`,
          {
            headers: {
    ...
    const results = assets
      ...
      .map((a: any) => {
        ...
        const nameIndicatesETF = nameLower.includes(' etf') || 
                                 nameLower.endsWith('etf') ||
                                 nameLower.includes('ishares') ||
                                 nameLower.includes('spdr') ||
                                 nameLower.includes('vanguard') && (nameLower.includes('fund') || nameLower.includes('etf'));
    ...
    if (results.length > 0) {
      console.log(`   Top results: ${results.slice(0, 3).map((r: any) => `${r.symbol} (${r.name})`).join(', ')}`);
    }
...
app.get("/make-server-cbef71cf/crypto/search", async (c) => {
  try {
    ...
    const { data: coinGeckoProvider } = await supabase
      .from("data_providers")
...
app.post("/make-server-cbef71cf/ai-providers", async (c) => {
  try {
    ...
    console.log("📝 Creating AI provider:", { name: body.name, providerName: body.providerName });
    ...
    return c.json({ success: true, provider: formatAIProvider(data) });
  } catch (error) {
...
app.get("/make-server-cbef71cf/ai-providers/:id/reveal-key", async (c) => {
  try {
    ...
    return c.json({ apiKey: data?.api_key || '' });
  } catch (error) {
...
app.get("/make-server-cbef71cf/ai-providers/:id/reveal-secret", async (c) => {
  try {
    ...
    return c.json({ apiSecret: data?.api_secret || '' });
  } catch (error) {
...
app.get("/make-server-cbef71cf/crypto-old", async (c) => {
  try {
    ...
    const cryptos = (data || []).map((row: any) => ({
      id: row.symbol,
      cgId: row.symbol,
      symbol: row.symbol,
      name: row.name,
...
app.get("/make-server-cbef71cf/stocks/custom-names/all", async (c) => {
  try {
    ...
    return c.json({ ok: true, customNames });
  } catch (error) {
    console.error("Error fetching custom names:", error);
    return jsonErr(c, 500, 'CUSTOM_NAMES_FETCH_FAILED', error);
  }
...
app.post("/make-server-cbef71cf/stocks/refresh", async (c) => {
  try {
    ...
    if (symbols.length === 0) {
      ...
      return c.json({ ok: true, message: "No stocks to refresh" });
    }
    ...
    for (const symbol of symbols) {
      try {
        ...
        const barsResponse = await fetch(
          `${ALPACA_DATA_BASE}/v2/stocks/${symbol}/bars?start=${startDate}&timeframe=1Day&limit=365`,
          {
            headers: {
              'APCA-API-KEY-ID': alpacaApiKey,
              'APCA-API-SECRET-KEY': alpacaApiSecret,
        ...
        if (barsResponse.ok) {
          ...
          if (bars.length > 0) {
            ...
            bars.sort((a: any, b: any) => new Date(a.t).getTime() - new Date(b.t).getTime());
            
            const latestBar = bars[bars.length - 1];
            const currentPrice = latestBar.c; // Use close price if we don't have live quote
            
            // Calculate 52-week high and low
            year_high = Math.max(...bars.map((b: any) => b.h));
            year_low = Math.min(...bars.map((b: any) => b.l));
...
app.post("/make-server-cbef71cf/crypto/refresh-old", async (c) => {
  try {
    ...
    const cryptoSymbols = (cryptos || []).map((c: any) => c.symbol.toLowerCase());
    
    if (cryptoSymbols.length === 0) {
      console.log("⚠️ No cryptos found to refresh");
      return c.json({ ok: true, message: "No cryptos to refresh" });
    }
    ...
    const response = await fetch(
      `https://api.coingecko.com/api/v3/coins/markets?ids=${idsParam}&vs_currency=usd&order=market_cap_desc&per_page=${cryptoSymbols.length}&page=1&sparkline=false&price_change_percentage=1h,24h,7d,30d`,
      {
        headers: {
    ...
    if (configured) {
      try {
        ...
        const testResponse = await fetch(
          `${ALPACA_TRADING_BASE}/v2/account`,
          {
            headers: {
              "APCA-API-KEY-ID": apiKey,
              "APCA-API-SECRET-KEY": apiSecret,
...
app.get("/make-server-cbef71cf/stocks/search-old", async (c) => {
  try {
    ...
    const queryLower = query.toLowerCase();
    const matchedAssets = allAssets.filter((asset: any) => {
    ...
    const results = matchedAssets.map((asset: any) => {
...
app.post("/make-server-cbef71cf/stocks/add", async (c) => {
  try {
    ...
    const assetResponse = await fetch(
      `${ALPACA_TRADING_BASE}/v2/assets/${symbol}`,
      {
        headers: {
          'APCA-API-KEY-ID': alpacaApiKey,
          'APCA-API-SECRET-KEY': alpacaApiSecret,
    ...
    const normalizeAssetType = (alpacaClass: string, assetName: string, assetAttributes?: string[]): string => {
...
app.delete("/make-server-cbef71cf/stocks/:symbol", async (c) => {
  try {
    ...
    return c.json({ ok: true, message: `Stock ${symbol} deleted` });
  } catch (error) {
...
app.delete("/make-server-cbef71cf/crypto/:id", async (c) => {
  try {
    ...
    return c.json({ ok: true, message: `Crypto ${id} deleted` });
  } catch (error) {
...
app.post("/make-server-cbef71cf/crypto/add-old", async (c) => {
  try {
    ...
    const response = await fetch(
      `https://api.coingecko.com/api/v3/coins/markets?ids=${id}&vs_currency=usd&order=market_cap_desc&per_page=1&page=1&sparkline=false&price_change_percentage=1h,24h,7d,30d`,
      {
        headers: {
          'X-CG-DEMO-API-KEY': cgApiKey,
        },
    ...
    const { data: cryptoEntry, error } = await supabase
      .from("alpaca_stocks")
      .insert({
    ...
    return c.json({ ok: true, crypto: cryptoEntry });
  } catch (error) {
...
app.delete("/make-server-cbef71cf/crypto-old/:id", async (c) => {
  try {
    ...
    return c.json({ ok: true, message: `Crypto ${symbol} deleted` });
  } catch (error) {
    console.error("Error deleting crypto:", error);
    return jsonErr(c, 500, 'CRYPTO_DELETE_FAILED', error);
  }
...
app.post("/make-server-cbef71cf/stocks/custom-name", async (c) => {
  try {
    ...
    const { data, error } = await supabase
      ...
      .update(updateData)
      .eq("symbol", symbol)
      .select("symbol, name, custom_name")
      .single();
...
app.get("/make-server-cbef71cf/alpaca-stocks", async (c) => {
  try {
    ...
    return c.json({ ok: true, stocks: data || [] });
  } catch (error) {
...
app.post("/make-server-cbef71cf/crypto/add", async (c) => {
  try {
    ...
    return c.json({ ok: true, message: `${data.name} added successfully` });
  } catch (error) {
    console.error("Error adding crypto:", error);
    return jsonErr(c, 500, 'CRYPTO_ADD_FAILED', error);
  }
...
app.post("/make-server-cbef71cf/crypto/refresh", async (c) => {
  try {
    ...
    return c.json({ ok: true, refreshed: refreshedCount, total: cryptoSymbols.length });
  } catch (error) {
    console.error("Error refreshing cryptos:", error);
...
app.get("/make-server-cbef71cf/crypto", async (c) => {
  try {
    ...
    return c.json({ ok: true, cryptos: data || [] });
  } catch (error) {
