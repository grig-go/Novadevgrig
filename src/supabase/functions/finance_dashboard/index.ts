    ...
    for (const security of securities) {
      try {
        ...
        if (isCrypto) {
          ...
          const searchUrl = coinGeckoCredentials.apiKey
            ? `https://api.coingecko.com/api/v3/search?query=${encodeURIComponent(symbol)}&x_cg_demo_api_key=${coinGeckoCredentials.apiKey}`
          ...
          // Get market data
          const marketUrl = coinGeckoCredentials.apiKey
            ? `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${coin.id}&x_cg_demo_api_key=${coinGeckoCredentials.apiKey}`
          ...
          if (updateError) {
            ...
            failed++;
            errors.push({ symbol, error: updateError.message });
          } else {
        ...
        failed++;
        errors.push({ symbol: security.symbol, error: String(err) });
      }
    }
...
app.post("/stocks", async (c) => {
  try {
    ...
    const normalizedType = (() => {
      ...
      if (["EQUITY", "ETF", "INDEX", "CRYPTO"].includes(t)) return t;
      if (["US_EQUITY", "STOCK"].includes(t)) return "us_equity";
    ...
    const { data, error } = await supabase
      ...
      .upsert(
        ...
        { onConflict: "symbol" } // ✅ overwrite on symbol conflict
      )
      .select()
      .single();
...
app.put("/stocks/:symbol", async (c) => {
  try {
    ...
    if (body.change_1d_pct !== undefined)
      updateData.change_1d_pct = body.change_1d_pct;
    if (body.change_1w_pct !== undefined)
      updateData.change_1w_pct = body.change_1w_pct;
    if (body.change_1y_pct !== undefined)
      updateData.change_1y_pct = body.change_1y_pct;
...
app.get("/stocks/overrides/list", async (c) => {
  try {
    const { data, error } = await supabase
      .from("alpaca_stocks")
      .select("symbol, name, custom_name, type")
...
app.get("/chart/:symbol", async (c) => {
  try {
    ...
    const now = new Date();
    const resolutionConfig: Record<string, { timeframe: string; start: Date }> = {
      "1d": { 
        timeframe: "1Min", 
    ...
    const url = `https://data.alpaca.markets/v2/stocks/${symbol}/bars?timeframe=${config.timeframe}&start=${startDate}&end=${endDate}&limit=10000`;
  ...
  try {
    ...
    if (!coinGeckoCredentials) {
      return jsonErr(
        ...
        "CoinGecko API credentials not configured"
      );
    }
    ...
    const searchUrl = coinGeckoCredentials.apiKey
      ? `https://api.coingecko.com/api/v3/search?query=${encodeURIComponent(query)}&x_cg_demo_api_key=${coinGeckoCredentials.apiKey}`
    ...
    if (!searchResponse.ok) {
      ...
      return jsonErr(
        ...
        "COINGECKO_API_ERROR",
        `CoinGecko API error (${searchResponse.status}): ${errorText}`
      );
    }
    ...
    const marketsUrl = coinGeckoCredentials.apiKey
      ? `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${coinIds}&order=market_cap_desc&sparkline=false&x_cg_demo_api_key=${coinGeckoCredentials.apiKey}`
...
app.get("/crypto-price/:id", async (c) => {
  try {
    ...
    const url = coinGeckoCredentials.apiKey
      ? `https://api.coingecko.com/api/v3/simple/price?ids=${id}&vs_currencies=usd&include_24hr_change=true&x_cg_demo_api_key=${coinGeckoCredentials.apiKey}`
    ...
    if (!response.ok) {
      ...
      return jsonErr(
        c,
        response.status,
        "COINGECKO_PRICE_ERROR",
        `CoinGecko price error: ${errorText}`
      );
    }
