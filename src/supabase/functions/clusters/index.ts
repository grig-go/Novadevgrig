...
app.get("/health", (c) => {
  return c.json({ status: "ok", service: "clusters" });
});
...
app.get("/stats", async (c) => {
  try {
    ...
    const { data: clusters } = await supabase
      .from("news_clusters")
      .select("category, sentiment, article_count");
    ...
    clusters?.forEach((cluster) => {
      if (cluster.category) {
        categoryCount[cluster.category] =
          (categoryCount[cluster.category] || 0) + 1;
      }
      if (cluster.sentiment) {
        sentimentCount[cluster.sentiment] =
          (sentimentCount[cluster.sentiment] || 0) + 1;
      }
    ...
    console.error(
      "[CLUSTERS] Unexpected error fetching stats:",
      error,
    );
    return c.json(
      {
...
app.post("/bulk-delete", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error bulk deleting clusters:",
        error,
      );
      return c.json(
        {
    ...
    console.log(
      "[CLUSTERS] Bulk delete successful:",
      ids.length,
      "clusters",
    );
    ...
    console.error(
      "[CLUSTERS] Unexpected error bulk deleting:",
      error,
    );
    return c.json(
      {
...
app.get("/", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error fetching clusters:",
        error,
      );
      return c.json(
        {
    ...
    console.error(
      "[CLUSTERS] Unexpected error fetching clusters:",
      error,
    );
    return c.json(
      {
...
app.post("/", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error creating cluster:",
        error,
      );
      return c.json(
        {
    ...
    console.log(
      "[CLUSTERS] Cluster created successfully:",
      cluster.id,
    );
    return c.json({ cluster }, 201);
  } catch (error) {
    console.error(
      "[CLUSTERS] Unexpected error creating cluster:",
      error,
    );
    return c.json(
      {
...
app.get("/:id", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error fetching cluster:",
        error,
      );
      return c.json(
        {
    ...
    console.error(
      "[CLUSTERS] Unexpected error fetching cluster:",
      error,
    );
    return c.json(
      {
...
app.put("/:id", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error updating cluster:",
        error,
      );
      return c.json(
        {
    ...
    console.error(
      "[CLUSTERS] Unexpected error updating cluster:",
      error,
    );
    return c.json(
      {
...
app.delete("/:id", async (c) => {
  try {
    ...
    if (error) {
      console.error(
        "[CLUSTERS] Error deleting cluster:",
        error,
      );
      return c.json(
        {
    ...
    console.error(
      "[CLUSTERS] Unexpected error deleting cluster:",
      error,
    );
    return c.json(
      {
