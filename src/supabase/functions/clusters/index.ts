...
app.get('/clusters', async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to fetch clusters', details: error.message }, 500);
    }
...
app.get('/clusters/:id', async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to fetch cluster', details: error.message }, 500);
    }
...
app.post('/clusters', async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to create cluster', details: error.message }, 500);
    }
...
app.put('/clusters/:id', async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to update cluster', details: error.message }, 500);
    }
...
// Delete cluster
app.delete('/clusters/:id', async (c) => {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to delete cluster', details: error.message }, 500);
    }

    console.log('[CLUSTERS] Cluster deleted successfully:', id);
    return c.json({ success: true, message: 'Cluster deleted' });
  } catch (error) {
...
app.post('/clusters/bulk-delete', async (c) => {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to delete clusters', details: error.message }, 500);
    }
...
app.get('/clusters/stats', async (c) => {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
